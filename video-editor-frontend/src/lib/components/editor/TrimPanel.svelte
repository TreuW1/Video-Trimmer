<script lang="ts">
  ...
  function presetSummary(mode: CompressionPreset): string {
    if (mode.processingStrategy === 'sizeTarget') return mode.sizeLabel ?? `~${mode.sizeLimitMB ?? mode.targetSizeMB} MB`;
...
>
  {#if controlsEnabled}
    <div class="flex-1 overflow-auto p-4">
      ...
      <div class="grid grid-cols-2 gap-2 mb-5">
        <button
          ...
          class="p-2 border rounded-lg font-medium cursor-pointer text-xs uppercase bg-gradient-to-r from-teal-600/40 to-teal-500/40 border-teal-500/50 text-teal-300 hover:bg-teal-500/30 disabled:opacity-50 disabled:cursor-not-allowed shadow-md"
        >
          Set Start (S)
        </button>
        ...
        <button
          ...
          disabled={!controlsEnabled}
          class="p-2 border rounded-lg font-medium cursor-pointer text-xs uppercase bg-gradient-to-r from-amber-600/40 to-amber-500/40 border-amber-500/50 text-amber-300 hover:bg-amber-500/30 disabled:opacity-50 disabled:cursor-not-allowed shadow-md"
        >
          Set End (E)
        </button>
      </div>
      ...
      <button
        ...
        disabled={!controlsEnabled}
        class="w-full p-3 mb-5 rounded-lg font-medium cursor-pointer transition-all duration-200 text-sm shadow-md bg-gradient-to-r from-slate-600/50 to-gray-700/50 border border-gray-700/30 text-white hover:from-slate-500/50 hover:to-gray-600/50 disabled:opacity-50 disabled:cursor-not-allowed"
      >
        Play from Start (P)
      </button>
      ...
      <div class="mb-5">
        <div class="mb-3 flex items-center justify-between gap-2">
          ...
          <div class="flex gap-1">
            <button
              type="button"
              class="rounded-md border border-gray-600/40 px-2 py-1 text-xs text-slate-300 hover:bg-gray-800/60"
            ...
            <button
              type="button"
              class="rounded-md border border-gray-600/40 px-2 py-1 text-xs {managePresets ? 'bg-slate-600 text-white' : 'text-slate-300 hover:bg-gray-800/60'}"
              onclick={() => {
    ...
    <div class="p-3 border-t border-gray-700/20 bg-gray-900/30">
      ...
      >
        {selectedTrackCount > 1 ? 'Combine & Trim Videos' : 'Trim video'}
      </button>
    </div>
